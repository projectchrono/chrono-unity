...
public class CameraFollower : MonoBehaviour, IAdvance
{
    ...
    private void OnGUI()
    {
        ...
        GUI.Label(new Rect(10, 10, 200, 40), vehicle.name, guiStyle);

        double speedKPH = Math.Round(3.6 * vehicle.GetSpeed());
        GUI.Label(new Rect(10, 40, 200, 40), "Speed (km/h): " + speedKPH.ToString(), guiStyle);
        double throttle = Math.Round(vehicle.GetThrottleInput() * 100) / 100;
        GUI.Label(new Rect(10, 70, 200, 40), "Throttle: " + throttle.ToString(), guiStyle);
        double braking = Math.Round(vehicle.GetBrakingInput() * 100) / 100;
        GUI.Label(new Rect(10, 90, 200, 40), "Braking: " + braking.ToString(), guiStyle);
        double steering = Math.Round(vehicle.GetSteeringInput() * 100) / 100;
        GUI.Label(new Rect(10, 110, 200, 40), "Steering: " + steering.ToString(), guiStyle);

        switch (vehicle.GetChVehicle().GetPowertrain().GetDriveMode())
        {
            case ChPowertrain.DriveMode.FORWARD:
                GUI.Label(new Rect(10, 130, 200, 40), "Gear: Forward", guiStyle);
                break;
            case ChPowertrain.DriveMode.REVERSE:
                GUI.Label(new Rect(10, 130, 200, 40), "Gear: Reverse", guiStyle);
                break;
        }

        double motorTorque = Math.Round(vehicle.GetPowertrain().GetMotorTorque());
        GUI.Label(new Rect(10, 160, 200, 40), "Motor Torque (Nm): " + motorTorque.ToString(), guiStyle);
        double motorSpeed = Math.Round(vehicle.GetPowertrain().GetMotorSpeed() * 60 / (2 * Math.PI));
        GUI.Label(new Rect(10, 180, 200, 40), "Motor Speed (RPM): " + motorSpeed.ToString(), guiStyle);

        float wallTime = Time.realtimeSinceStartup;
        float gameTime = Time.unscaledTime;
        float ratio = gameTime / wallTime;
        GUI.Label(new Rect(10, 200, 200, 40), "Time factor: " + Mathf.Round(ratio * 100) / 100, guiStyle);
    }
}
